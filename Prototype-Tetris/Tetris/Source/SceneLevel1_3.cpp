...
SceneLevel1_3::SceneLevel1_3(bool startEnabled) : Module(startEnabled)
{
	//Curtain anim
...
Update_Status SceneLevel1_3::PostUpdate()
{
	...
	App->fonts->BlitText(175, 210, BlueFont, "3");
...
void SceneLevel1_3::MinMaxSort(int a[], int n) {
	int i, j, min, temp;
	for (i = 0; i < n - 1; i++) {
		min = i;
		for (j = i + 1; j < n; j++)
...
void SceneLevel1_3::loser(const char* ch_losetoContinue) {
...
int SceneLevel1_3::ConstChartoInt(const char* x) { // Function that converts const char* to int
...
int SceneLevel1_3::StrToInt(string x) { // Function that converts string to int
	int temp = 0;
...
void SceneLevel1_3::RedScore(int value) {
...
void SceneLevel1_3::AddPlayer(string P, int score) {
	bool isEmpty = false;
	fstream ScoreSave;
...
void SceneLevel1_3::SortPlayers() {
	fstream ScoreSave;
	int o = 0;
	ScoreSave.open("ScoreN.txt", ios::in);
	if (ScoreSave.is_open()) {
...
string SceneLevel1_3::Sorter(string Player1) {
	int ACF = Currentpos;
	int CF = 0;
	for (int i = 0; i < 16; i++) {
...
void SceneLevel1_3::Score(int score) {
	fstream Score;
	int i = 0;
