...
bool SceneLevel3_3::Start()
{
	...
	App->tetromino->speed = App->tetromino->speed9;
...
Update_Status SceneLevel3_3::Update()
{
	...
	if (App->input->keys[SDL_SCANCODE_ESCAPE] == Key_State::KEY_DOWN || pad.r2) {
	...
	//Winner hotkey
	if (App->input->keys[SDL_SCANCODE_F1] == Key_State::KEY_DOWN || pad.start)
	{
...
void SceneLevel3_3::MinMaxSort(int a[], int n) {
	int i, j, min, temp;
	for (i = 0; i < n - 1; i++) {
		min = i;
		for (j = i + 1; j < n; j++)
...
void SceneLevel3_3::Lines(int lines) {
